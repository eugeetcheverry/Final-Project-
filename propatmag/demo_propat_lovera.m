...
kepel = [7000000, 0.01, 98*pi/180, 0, 0, 0];   % see function delkep
...
kepel = [6.378135e06 + 450000, 0.0, 87*pi/180, 0, 0, 0];
...
w_ang = 0.1*[0.1, 0, 0.5]' + sqrt(2)*dlyap*[1;-1;1]/sqrt(3);           % in radians/sec
...
%iner = [7 2 1; 2 10 0;1 0 15];         % in kg*m*m
...
floquet = 1; % 0=NO armar matriz fundamental / 1:6=calc. col de Matriz Fundamental.
if floquet
    w_ang = [0 0 0]';
    quat  = [0 0 0 1]';
...
for t = tstart:tstep:tend
    ...
    ambt = [0 0 0]';
    A    = quatrmx(quat);
    ambt = 0*3*omeg_0^2*cross(A*stat(1:3)',iner*A*stat(1:3)')/norm(stat(1:3))^2;
    ...
    if flag_mag == 0
        ...
        % Magnetic Control
        k_p = 300;            % ganancia proporcional
        k_v = 300;            % ganancia derivativa
        eps = 2.324E-3;           % epsilon
        ...
        u = -eps*eps*k_p*dq*signq4 - eps*k_v*iner*dw - ambt*0;
        u = - invin*(eps*eps*k_p*dq*signq4 + eps*k_v*dw);
...
if floquet
    ...
    save(['floquet_' num2str(floquet) '.mat'], 'x_floq')
    %return
end
