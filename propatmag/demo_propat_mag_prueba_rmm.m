...
iner = [0.0021 0 0; 0 0.0019 0; 0 0 0.002];         % in kg*m*m
...
xhatkmi = [w_ang*0;maxmagmom*[0.003;0.003;-0.003]*0];
xhatk   = [w_ang*0;maxmagmom*[0.003;0.003;-0.003]*0];
vxhat   = [xhatk];
Pk      = [[eye(3)*0.00001^2 zeros(3)];[zeros(3) eye(3)*0.005^2]];
Akm     = zeros(6);
Gamk    = tstep*eye(6);
Qk      = eye(6)*0.001^2;
...
for t = tstart:tstep:tend
    ...
    if flag_mag == 0
        ...
        R       = eye(3)*0.1^2;
        Akm     = [[inv(iner)*( Skew(iner*wref) - Skew(wref)*iner ) [-inv(iner)*Skew(earth_field_b)]];zeros(3,6)];
        Bkm     = eye(6);
        
        Phik    = expm(Akm*tstep);
        xhatkmi = Phik*xhatk + Gamk*[inv(iner)*1/normb2 * cross(cross(earth_field_b, exu),earth_field_b);zeros(3,1)];  % cross(mag_mom, earth_field_b)
        ...
        %wref = wref + xhatk(1:3);
        %xhatk(1:3) = 0*xhatk(1:3);
        
        exu=u;
        
        % Simulo momento magnético
        mag_mom = 1/normb2 * cross(earth_field_b, u) - xhatk(4:6)*compensarmm;
        ...
        mag_mom = mag_mom + maxmagmom*[0.003;0.003;-0.003]*simularmm;
...
figure(999);clf;
plot(time/orb_period,vxhat(4,:)','r');hold on;
plot(time/orb_period,0.0003*ones(length(time),1),'r.-');hold on;
plot(time/orb_period,vxhat(5,:)','g');hold on;
plot(time/orb_period,0.0003*ones(length(time),1),'g--');hold on;
plot(time/orb_period,vxhat(6,:)','b');hold on;
plot(time/orb_period,-0.0003*ones(length(time),1),'b--');hold on;
title('Magnetic Moment Residual Estimation');