...
eulzxz = [180, 0, 0]'*pi/180;   % converted from degrees to radians
...
w_ang = [0.2, 0.2, -0.2]'/8.*0 + sqrt(2)*dlyap*[1;-1;1]/sqrt(3);           % in radians/sec

%w_ang = [0.0;0;0]+sqrt(2)*dlyap*[1;-1;1]/sqrt(3);
%quat = [0;0;0;1];

%quat = [-0.262880731177140  -0.090059431878542   0.203340154447831   0.938848124828840]';
%w_ang = [0.396964449924616   0.246626618410139  -0.324362332894472]'*1e-4;
...
tstep = 60;             % step time (sec)
tend = 2*15*orb_period; % end time (10 minutes)
...
iner = [0.3 0.01 0.01;0.01 0.3 0;0.01 0 0.3];      % in kg*m*m
...
maxmagmom = 0.1; % ex 2     % Tope momento magnético en Am2
vdq = [0;0;0];
vdw = [0;0;0];
u = [0;0;0];
vu = u;
...
for t = tstart:tstep:tend
    ...
    if flag_mag == 0
        ...
        k_p = 0.001;
        k_v = 0.5;                                  % ganancia derivativa
        eps = 0.0282357 + 0*0.000005 - 0.0182357;   % epsilon
        eps = 0.001*50;   % epsilon
        ...
        %if (sqrt(normb2)<(0.00175*1.0)*5.359835274750961e-05)
        %  mag_mom = 0.*mag_mom;
        %end;
...
figure(4000)
subplot(2, 1, 1);
plot(time/orb_period, orbit(1:3,:)/1000);hold on;
plot(time/orb_period, (orbit(1,:).^2+orbit(2,:).^2+orbit(3,:).^2).^0.5/1000,'k');
xlabel('Time (orbits)')
ylabel('Position (km)')
title('Satellite inertial position ')

subplot(2, 1, 2);
plot(time/orb_period, orbit(4:6,:)/1000);hold on;
plot(time/orb_period, (orbit(4,:).^2+orbit(5,:).^2+orbit(6,:).^2).^0.5/1000,'k');
xlabel('Time (orbits)')
...
Nb=500;
c=zeros(2*Nb,2*Nb,2*Nb);
for iq=1:length(vdq)
    c(round(vdq(:,iq)*Nb+Nb))=c(round(vdq(:,iq)*Nb+Nb))+1;
end;
...
figure(2210);clf;
plot3(vdq(1,tini:end),vdq(2,tini:end),vdq(3,tini:end));grid on;
title('Quaternion: vector part')
grid on

figure(2201);clf;
plot3(vdw(1,tini:end),vdw(2,tini:end),vdw(3,tini:end));grid on;
title('Anular velocity [rad/sec]');
axis([-3 3 -3 3 -3 3]*1e-4);
grid on;
...
figure(222220);
plot(asin(sqrt(vdq(1,tini:end).^2+vdq(2,tini:end).^2+vdq(3,tini:end).^2))*2*180/pi, (180/pi)*sqrt(vdw(1,tini:end).^2+vdw(2,tini:end).^2+vdw(3,tini:end).^2), 'k');hold on;
title('Norma de la velocidad angular [o/s] vs. ángulo de error [o]');hold on;xlabel('Ángulo [o]');ylabel('Velocidad [o/s]');grid on;hold on;
...
title('Logaritmo de la norma de las diferencias entre soluciones');hold on;
xlabel('Órbitas');
plot(time/orb_period,logdif,'k');hold on;grid on;


vdw2=vdw(1,((1+251):end));
vdw3=vdw(1,((1+251*2):end));

figure(231);
plot3(vdw(1:length(vdw3)),vdw2(1:length(vdw3)),vdw3(1:length(vdw3)));
%plot(vdw(1:length(vdw3)),vdw2(1:length(vdw3)));
...
x=vdw(1,3:end);
y=acorr(x);
title('Correlación de la velocidad');hold on;
