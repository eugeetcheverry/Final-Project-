...
w_ang = [0.1, 0, 0.1]' + sqrt(2)*dlyap*[1;-1;1]/sqrt(3);           % in radians/sec
...
for t = tstart:tstep:tend
    ...
    omeg_0 = 0; % Canela gradiente gravitatoroi
    
    % Perturbation torques:
    ambt = [0 0 0]';
    A    = quatrmx(quat);
    ambt = 3*omeg_0^2*cross(A*stat(1:3)',iner*A*stat(1:3)')/norm(stat(1:3))^2;
    ...
    if flag_mag == 0
        ...
        % Magnetic Control
        k_p = 0.02;
        k_p = 0.02*max(max(iner));            % ganancia proporcional
        ...
        %u = -eps*eps*k_p*dq*signq4 - eps*k_v*iner*dw - ambt + cross(dw,iner*dw);
        %gamavg = gamavg + Skew(earth_field_b)*Skew(-earth_field_b);
        %gamavg = gamavg / norm(gamavg);
        %u = pinv(gamavg)*u;
        
        ALin = [-Skew(earth_field_b)*Skew(-earth_field_b)*[-eps*eps*k_p*eye(3) -eps*k_v*iner];zeros(3) eye(3)/2];

        %Wq = [[quat(4) -quat(3) quat(2)];[quat(3) quat(4) -quat(1)];[-quat(2) quat(1) quat(4)];[-quat(1) -quat(2) -quat(3)]]/2;
        %delta = delta + tstep*(quat-eps*lambda*delta);
        %u = -eps*eps*k_p*dq*signq4 - k_v*alfa*lambda*Wq'*(quat-eps*lambda*delta);

        AK  = quatrmx(quat)*k_p*(iner/norm(iner));
        sKA = (AK-AK')/2;
        eKA = -[sKA(3,2);sKA(1,3);sKA(2,1)];
        k_p = 0.01;            % ganancia proporcional
        k_v = 0.03;            % ganancia derivativa
        eps = 0.05;            % epsilon
        uG  = -inv(iner)*( eps*eps*eKA*signq4 + eps*k_v*iner^2*dw );

        mus = 1;
        ke  = 2;
        ks  = mus*exp(-ke*(earth_field_b'/norm(earth_field_b)*uG/(norm(uG)+1e-12))^2);

        %u = ks * uG - ambt + cross(dw,iner*dw);
...
figure(222);clf;
subplot(2,1,1);
plot(time(1,tini:end), 1000*vext_torq(1,tini:end), 'r');hold on;
plot(time(1,tini:end), 1000*vext_torq(2,tini:end), 'g');hold on;
plot(time(1,tini:end), 1000*vext_torq(3,tini:end), 'b');hold on;
title('Magnetorquers Control Magnetic Torque [mNm]')
subplot(2,1,2);
plot(time(1,tini:end), vmag_mom(1,tini:end), 'r');hold on;
plot(time(1,tini:end), vmag_mom(2,tini:end), 'g');hold on;
plot(time(1,tini:end), vmag_mom(3,tini:end), 'b');hold on;
title('Magnetorquers Control Magnetic Moment [Am2]')
grid on


figure(223);clf;
plot(time(1,tini:end), veKA(1,tini:end)/k_p, 'r');hold on;
plot(time(1,tini:end), veKA(2,tini:end)/k_p, 'g');hold on;
plot(time(1,tini:end), veKA(3,tini:end)/k_p, 'b');hold on;
title('Attitude error eKA/kp')
grid on
...
plot(logdif,'k');hold on;grid on;

end

if 0

tini = ceil(length(vdq)*1/2);
tini = 1;
xx = [vdw(:,tini:end)]';
[P, t] = poincare_map(xx);
...
for i = 0:100
    for j = 0:100
        for k = 0:100
            quat13  = [i/50-0.5;j/50-0.5;k/50-0.5]/sqrt(3);
            quat4   = sqrt(1-norm(quat13)^2);
            quat14  = [quat13;quat4];
            AK      = quatrmx(quat14)*k_p*(iner/norm(iner));
            sKA     = (AK-AK')/2;
            eKA     = -[sKA(3,2);sKA(1,3);sKA(2,1)];
            angle   = acos(eKA'*quat13/norm(quat13)/norm(eKA))*180/pi;
            gain    = norm(eKA)/k_p;
            vangle  = [vangle angle];
            vgain   = [vgain gain];
            vijk    = [vijk [i;j;k]];
            vquat14 = [vquat14 quat14];
        end
    end
end

figure(1000);clf;
plot3(vgain.*vquat14(1,:),vgain.*vquat14(2,:),vgain.*vquat14(3,:),'k.');hold on;

figure(1002);clf;
plot3(vgain.*vangle.*vquat14(1,:),vgain.*vangle.*vquat14(2,:),vgain.*vangle.*vquat14(3,:),'k.');hold on;

veps = 0.0005:0.0005:0.01;
veigAL = [];
for iii = 1:length(veps)
    eps    = veps(iii);
    ALin   = [[zeros(3) eye(3)/2];-inv(iner)*Skew(earth_field_b/norm(earth_field_b))*Skew(-earth_field_b/norm(earth_field_b))*[eps*eps*k_p*eye(3) eps*k_v*iner]];
    %ALin   = [[zeros(3) eye(3)/2];-[eps*eps*k_p*eye(3) eps*k_v*iner]];
    veigAL = [veigAL real(eig(ALin))];
end
figure(2000);clf;
plot(veps,veigAL(1,:),'r');hold on;
plot(veps,veigAL(2,:),'g');hold on;
plot(veps,veigAL(3,:),'b');hold on;
plot(veps,veigAL(4,:),'k');hold on;
plot(veps,veigAL(5,:),'m');hold on;
plot(veps,veigAL(6,:),'c');hold on;
