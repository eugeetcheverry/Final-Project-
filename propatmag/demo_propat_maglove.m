...
w_ang = [0.01, 0, 0]' + sqrt(2)*dlyap*[1;-1;1]/sqrt(3);           % in radians/sec
...
tstep = 2*120;             % step time (sec)
tend = 2*8*15*orb_period; % end time (10 minutes)
...
maxmagmom = 2;      % Tope momento magnético en Am2
vdq = [0;0;0];
vdw = [0;0;0];
u = [0;0;0];
vu = u;
...
for t = tstart:tstep:tend
    ...
    if flag_mag == 0
        ...
        eps = 0.0282357 + 0*0.000005 - 0*0.0182357;   % epsilon
        eps = 0.015;
        %eps = 0.024;
        %eps = 0.026;
        %eps = 0.0282357;
        ...
        u = -eps*eps*k_p*inv(iner)*dq*signq4 - eps*k_v*iner*dw - ambt*0 + 0*cross(dw,iner*dw);
        ...
        if (sqrt(normb2)<(0.00175*(250.*0+0.35+1.5*0))*5.359835274750961e-05)  % 450 es el maximo para estabilidad
          mag_mom = 0.*mag_mom;
        end;
...
figure(222220);
plot(asin(sqrt(vdq(1,tini:end).^2+vdq(2,tini:end).^2+vdq(3,tini:end).^2))*180/pi, (180/pi)*sqrt(vdw(1,tini:end).^2+vdw(2,tini:end).^2+vdw(3,tini:end).^2), 'k');hold on;
...
vduty = [100.0     40  40.5  37  34.7   0.9];
vepsi = [0.0282357 0.027 0.017 0.024 0.0125  0.01];

vduty = [100.0       40    37     32.4   27.3   2.5];
vepsi = [0.0282357 0.028  0.024   0.011  0.0105 0.005];

vduty = [100.0       40    37     27.3     1.4];
vepsi = [0.0282357 0.028  0.024   0.0105 0.0075];
figure(1010);clf;
plot(vepsi,vduty,'k');hold on;
plot(vepsi,vduty,'ko');
xlabel('\epsilon');hold on;grid on;
ylabel('Orbital Duty Cycle [%]');hold on;
axis([0.0075 0.0282357 1.4 100]);
