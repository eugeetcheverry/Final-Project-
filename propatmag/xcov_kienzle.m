...
function [retval, lags] = xcov_kienzle(X, Y, maxlag, scale)
  ...
  if (!isempty(Y))
    [retval, lags] = xcorr_kienzle(center(X), center(Y), maxlag, scale);
  else
    [retval, lags] = xcorr_kienzle(center(X), maxlag, scale);
  endif
