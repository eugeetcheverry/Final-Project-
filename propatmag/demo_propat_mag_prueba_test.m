...
RE = 6378000;
kepel = [RE+620000, 0.01, 98, 0, 0, 0];     % see function delkep
% Orbit state vector:
...
w_ang = (1-floquet)*[10, 10, 10]'*pi/180 + floquet * [0;0;1]*pi/180;           % in radians/sec
...
tend = (10*(1+0.5*floquet))*orb_period;    % end time (10 minutes)
...
for t = tstart:tstep:tend
    ...
    if flag_mag == 0
        ...
        if (quatrmx(quat)*sun_dir(mjd, dfra+t))'*[0;0;1]>0.9 && t>4*orb_period && abs(stat(3))<100000
            nadirpointing=1;
        end;
...
plot(time(1:length(vanglexv))/orb_period,2*(180/pi)*vanglexv,'k');hold on;
ylabel('[deg]');
grid on;
xlabel('Time [orbits]');
