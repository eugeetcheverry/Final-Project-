...
w_ang = [0.2, 0.2, -0.2]' + sqrt(2)*dlyap*[1;-1;1]/sqrt(3);           % in radians/sec
...
figure(2222221);
plot(time(1,tini:end)/orb_period, (1./(time(1,tini:end)/orb_period)).*log(sqrt(vdq(1,tini:end).^2+vdq(2,tini:end).^2+vdq(3,tini:end).^2+vdw(1,tini:end).^2+vdw(2,tini:end).^2+vdw(3,tini:end).^2)), 'm');hold on;
title('1/t*log(|x|)');hold on;xlabel('Órbitas');grid on;hold on;
