...
% GNSS
GNSS_pos_error = 0.1;                 % position estimator error [m]
GNSS_vel_error = 0.002;                 % velocity estimator error [m/s]
...
% Orbit control
orbit_control_tau      = 60;        % [s]
ROE_alpha_gain         = 0.5*0.5*max_thrust/0.00005*1000000/50/5.5; % 0 cont�nuo, 1 pulsado
ROE_alpha_gain_horizon = 0.5*0.75;

if continuous ==1
    ROE_alpha_gain     = 0.5*0.5*max_thrust/0.00005*1000000/50/5.5*0.1;
    ROE_alpha_gain_horizon = 0.5*0.75;
end;
...
tstep_orbit    = orbit_control_tau*1;   % time step [s] % antes *3
tend_orbit     = 24*3600*8;         % end time [s]
