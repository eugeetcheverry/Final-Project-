...
int adcsMagSunf(struct AcType *AC)
{
   ...
   double eps = 0.01*40;               // Averaging Parameter
   double thnw = 0.003;                // End of Detumbling Velocity Condition [rad/sec] 
   double nm = 0.;                     // Auxiliary Variable
   ...
   if (mbvb > 0.) {
      ...
      for (int i = 0; i < 3; i++) {
         ...
         if (sumillum>threclipse) {
            ...
            if (kq!=0.) {
               ...
               for(i=0;i<3;i++) FindPDGains(AC->MOI[i][i],0.05,1.0, &C->Kr[i],&C->Kp[i]);
               for(i=0;i<3;i++) {
                  AC->Tcmd[i] = -C->Kr[i]*AC->wbn[i]-C->Kp[i]*(2.0*qc[i]);
               }
               if ( fabs(AC->Tcmd[0]) >  AC->Whl[0].Tmax || fabs(AC->Tcmd[1]) >  AC->Whl[1].Tmax || fabs(AC->Tcmd[2]) >  AC->Whl[2].Tmax) {
                  AC->Tcmd[0] =  -0.5*C->Kr[0]*AC->wbn[0]-C->Kp[0]*(2.0*(qc[0]));
                  AC->Tcmd[1] =  -0.5*C->Kr[1]*AC->wbn[1]-C->Kp[1]*(2.0*(qc[1]));
                  AC->Tcmd[2] =  -0.5*C->Kr[2]*AC->wbn[2]-C->Kp[2]*(2.0*(qc[2]));
               }
               if ( fabs(AC->Tcmd[0]) >  AC->Whl[0].Tmax || fabs(AC->Tcmd[1]) >  AC->Whl[1].Tmax || fabs(AC->Tcmd[2]) >  AC->Whl[2].Tmax) {
                  AC->Tcmd[0] =  -0.25*C->Kr[0]*AC->wbn[0]-C->Kp[0]*(2.0*(qc[0]));
                  AC->Tcmd[1] =  -0.25*C->Kr[1]*AC->wbn[1]-C->Kp[1]*(2.0*(qc[1]));
                  AC->Tcmd[2] =  -0.25*C->Kr[2]*AC->wbn[2]-C->Kp[2]*(2.0*(qc[2]));
               }
               if ( fabs(AC->Tcmd[0]) >  AC->Whl[0].Tmax || fabs(AC->Tcmd[1]) >  AC->Whl[1].Tmax || fabs(AC->Tcmd[2]) >  AC->Whl[2].Tmax) {
                  AC->Tcmd[0] =  -0.125*C->Kr[0]*AC->wbn[0]-C->Kp[0]*(2.0*(qc[0]));
                  AC->Tcmd[1] =  -0.125*C->Kr[1]*AC->wbn[1]-C->Kp[1]*(2.0*(qc[1]));
                  AC->Tcmd[2] =  -0.125*C->Kr[2]*AC->wbn[2]-C->Kp[2]*(2.0*(qc[2]));
               }
            } else if (nw < thnw) {
               AC->Tcmd[i] = - kw * eps * J[i] * AC->wbn[i];
      ...
      if (secondspointed>5000 && AC->GPS[0].Valid && AC->qbn[0]*AC->qbn[0]+AC->qbn[1]*AC->qbn[1]+AC->qbn[2]*AC->qbn[2]!=0) {
         retval = 1;
         printf("\n RETVAL=1 ss = %d, sp = %d ", secondssun, secondspointed);
      }
...
int adcsRwTriadTLEf(struct AcType *AC)
{
   ...
   int secondsmistop=300;
   int secondsnmistop=5000;
   int transitiontop=60;
...
/* .. Attitude Control Reference */
   ...
   double mbvb = MAGV(AC->bvb);        // Norm of B
   static double kq = 0.075, kw = 10.;    // Initial Gains
   int coriolis = 0;                   // == 1 => coriolis compensation
   static double earthvel = 0.0;       // != 0 => M3,                   earthvel * wo
   static double earthang = 0.0;       // != 0 => M3,                   earthvel * wo
   double kw0 = 4.0;   // =4           // Derivative Gain with Eclipse
   double kw1 = 5.0;                   // Derivative Gain without Eclipse
   double kq0 = 0.075; //*200;     // Proportional Gain with Eclipse
   double kq1 = 0.0075;             // Proportional Gain without Eclipse
   double eps = 0.001; // *4           // Averaging Parameter
   if (secondsnmis>0) {kq0=0.075*300;eps = 0.5*4.0;}
   double thnw = 0.001;                // End of Detumbling Velocity Condition [rad/sec] 
   double nm=0.;                       // Auxiliary Variable
   double threclipse = 0.5;            // Eclipse detection threshold (between 0 and 6, 6 never detects daylight, 0 never detects eclipse)
   double J[3] = {AC->MOI[0][0], AC->MOI[1][1], AC->MOI[2][2]};   // Inertia Matrix Diagonal
   ...
   if (mbvb > 0.) {
      ...
      for (int i = 0; i < 3; i++) {
         ...
         AC->Tcmd[i] = 0.01*AC->Tcmd[i];
      }
      for(i=0;i<3;i++) FindPDGains(AC->MOI[i][i],0.035,1.0, &C->Kr[i],&C->Kp[i]);
      if (secondsmis>0) for(i=0;i<3;i++) FindPDGains(AC->MOI[i][i],0.2,1.0, &C->Kr[i],&C->Kp[i]);
      for(i=0;i<3;i++) {
         AC->Tcmd[i] = -C->Kr[i]*AC->wbn[i]-C->Kp[i]*(2.0*(qc[i]+sqc[i]*2.0));
      }
      printf("\n ** 2*C->Kp[1] = %f",2*C->Kp[1]);
      
      if ( fabs(AC->Tcmd[0]) >  AC->Whl[0].Tmax || fabs(AC->Tcmd[1]) >  AC->Whl[1].Tmax || fabs(AC->Tcmd[2]) >  AC->Whl[2].Tmax) {
         AC->Tcmd[0] =  -0.5*C->Kr[0]*AC->wbn[0]-C->Kp[0]*(2.0*(qc[0]+sqc[0]*2.0));
         AC->Tcmd[1] =  -0.5*C->Kr[1]*AC->wbn[1]-C->Kp[1]*(2.0*(qc[1]+sqc[1]*2.0));
         AC->Tcmd[2] =  -0.5*C->Kr[2]*AC->wbn[2]-C->Kp[2]*(2.0*(qc[2]+sqc[2]*2.0));
      }
      if ( fabs(AC->Tcmd[0]) >  AC->Whl[0].Tmax || fabs(AC->Tcmd[1]) >  AC->Whl[1].Tmax || fabs(AC->Tcmd[2]) >  AC->Whl[2].Tmax) {
         AC->Tcmd[0] =  -0.25*C->Kr[0]*AC->wbn[0]-C->Kp[0]*(2.0*(qc[0]+sqc[0]*2.0));
         AC->Tcmd[1] =  -0.25*C->Kr[1]*AC->wbn[1]-C->Kp[1]*(2.0*(qc[1]+sqc[1]*2.0));
         AC->Tcmd[2] =  -0.25*C->Kr[2]*AC->wbn[2]-C->Kp[2]*(2.0*(qc[2]+sqc[2]*2.0));
      }
      if ( fabs(AC->Tcmd[0]) >  AC->Whl[0].Tmax || fabs(AC->Tcmd[1]) >  AC->Whl[1].Tmax || fabs(AC->Tcmd[2]) >  AC->Whl[2].Tmax) {
         AC->Tcmd[0] =  -0.125*C->Kr[0]*AC->wbn[0]-C->Kp[0]*(2.0*(qc[0]+sqc[0]*2.0));
         AC->Tcmd[1] =  -0.125*C->Kr[1]*AC->wbn[1]-C->Kp[1]*(2.0*(qc[1]+sqc[1]*2.0));
         AC->Tcmd[2] =  -0.125*C->Kr[2]*AC->wbn[2]-C->Kp[2]*(2.0*(qc[2]+sqc[2]*2.0));
      }
   ...
   { 
      if (first) {
            ...
            fprintf(FilePtr, "vm=[%f %f %f %f %f %f %f %f %d %d %f %f %f %f %f %f %f %f %f %f];\n", AC->qbr[0], AC->qbr[1], AC->qbr[2], AC->qbr[3], AC->Tcmd[0], AC->Tcmd[1], AC->Tcmd[2], phi, secondsmis, secondsnmis, phiq, dphiq, AC->qln[0], AC->qln[1], AC->qln[2], AC->qln[3], AC->qbn[0], AC->qbn[1], AC->qbn[2], AC->qbn[3]);
            fclose(FilePtr);
      } else {
            FilePtr = fopen("TSAT/missionf.m", "a");
            fprintf(FilePtr, "vm=[vm;%f %f %f %f %f %f %f %f %d %d %f %f %f %f %f %f %f %f %f %f];\n", AC->qbr[0], AC->qbr[1], AC->qbr[2], AC->qbr[3], AC->Tcmd[0], AC->Tcmd[1], AC->Tcmd[2], phi, secondsmis, secondsnmis, phiq, dphiq, AC->qln[0], AC->qln[1], AC->qln[2], AC->qln[3], AC->qbn[0], AC->qbn[1], AC->qbn[2], AC->qbn[3]);
            fclose(FilePtr);
      }
   }
